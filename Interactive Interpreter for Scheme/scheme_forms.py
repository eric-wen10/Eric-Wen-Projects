...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if isinstance(signature,list):
            Procedure.signature = expressions.rest
            return signature
        else:
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            """if expressions.rest == nil:
                return True
            while True:
                if expressions.first.first


                
                if scheme_eval(expressions.first.first, env)==False:
                    if expressions.first.rest:
                        return eval_all(expressions.first.first, env)
                    else:
                        return eval_all(expressions.first,env)
                elif expressions.rest  == nil:
                    return eval_all(expressions.first.rest)
                expressions = expressions.rest"""
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    Mu_function = MuProcedure(formals, expressions.rest)
    return Mu_function
    # END PROBLEM 11
