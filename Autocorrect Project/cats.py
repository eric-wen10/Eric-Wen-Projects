...
def pick(paragraphs, select, k):
    ...
    '''yes = [paragraphs[i] for i in paragraphs if select[i]]
    if k >= len[yes]:
        return ''
    return yes[k]'''
    ...
    if typed_word in word_list:
        ...
        for i in word_list:
            if diff_function(typed_word, i, limit) <= limit:
                list = list + [i]
                result = result + [diff_function(typed_word, i, limit)]
...
def feline_fixes(typed, source, limit):
    ...
    if typed == source:
        ...
        if len(typed) == len(source):
            return count + feline_fixes(typed[:-1], source[:-1], limit)
        else:
            return count + feline_fixes(typed[:-1], source[:-1], limit) + (abs(len(typed) - len(source))) 
    '''
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  
        ...
        # BEGIN
        # one is empty, exactly the same, first letter is same, over the limit
        #return 
        # END
    else:
...
def time_per_word(words, times_per_player):
    ...
    Arguments:
        words: a list of words, in the order th
        ey are typed.
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            divided_time_list = divided_time_list + [times_per_player[i][j+1] - times_per_player[i][j]]
        user_times = user_times + [divided_time_list]
    return match(words, user_times)
...
def fastest_words(match):
    ...
    for i in word_indices:
        for j in player_indices:
            min_player = min(player_indices, key = lambda j: time(match, j, i))
            winner_pre_list = winner_pre_list[0:min_player-1]+[get_word(match, i)]+winner_pre_list[min_player+1:0]
        winner_list[j] += [winner_pre_list]
    return winner_list"""
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            min_player = min(player_indices, key = lambda j: time(match, j, i))
            winner_pre_list = [get_word(match, i)]
        winner_list[min_player] += winner_pre_list
    return winner_list

    '''
    winner_list=[[]]
    
    for x in range(len(get_all_times(match))):
        winner_list += []
    ...
    for i in word_indices:
        for j in player_indices:
            min_player = min(player_indices, key = lambda j: time(match, j, i))
            winner_list[j] += [get_word(match, min_player)]
    return winner_list
    '''
    # END PROBLEM 10
