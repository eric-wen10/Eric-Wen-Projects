...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        """if self.damage and self.been_doubled ==False:
            if self.is_container==True:
                self.damage+=self.damage
                self.been_doubled ==True
                if not isinstance(self.ant_contained, FireAnt):
                self.ant_contained.damage+=self.ant_contained.damage
                self.ant_contained.been_doubled == True
            elif not isinstance(self,FireAnt):
                self.damage+=self.damage
                self.been_doubled ==True
            else:
                return None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive == False:
            if current_place.bees != [] and self.lower_bound<=self.x<=self.upper_bound :
        ...
        return None
        # return none if there is no bee in front of the ThrowerAnt in the tunnel
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew>0:
            ...
            return bee.reduce_health(bee.health)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained != None:
            #super().action(self.ant_contained)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        #doubling
        location_queen_ant=self.place
        while location_queen_ant.exit:
            ant_affected = location_queen_ant.exit.ant
            if ant_affected:
                if ant_affected not in self.list_ants_affected:
                    ant_affected.double()
                    self.list_ants_affected.append(ant_affected)
                if ant_affected.is_container==True:
                    ant_inside=ant_affected.ant_contained
                    if ant_inside and ant_inside not in self.list_ants_affected:
                        ant_inside.double()
                        self.list_ants_affected.append(ant_inside)
            location_queen_ant=location_queen_ant.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        self.freeze_time=0
        self.bee_action=Bee.action
    def throw_at(self, target):
        # BEGIN Problem EC
        if target is not None:
            self.freeze_time=5
            def slow_action(gamestate):
                if gamestate.time %2==0 and self.freeze_time > 0:
                    Bee.action(target, gamestate)
                    self.freeze_time-=1
                elif gamestate.time %2 != 0 and self.freeze_time > 0:
                    lambda x: None
                    self.freeze_time -=1
                else:
                    Bee.action(target, gamestate)
